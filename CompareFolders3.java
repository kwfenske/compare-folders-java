/*
  ...
  can:
     ...
     2. Create a checksum file for a folder.  This is the most efficient way of
        remembering the contents of a folder for later comparison.  After you
        back up files to external media, and later restore them, how will you
        know they are correct unless you have checksums from the original
        files?
     3. Update a checksum by assuming that files have not changed if they have
        ...
        internet.
     4. Find duplicate files with the same MD5 or SHA checksum.
  ...
  checksum and still be different (which can't be said about CRC32).  For
  documentation on XML, see:
  ...
  application to generate or test checksums for a single file.  See
  FindDupFiles to look for duplicate files by size and MD5 checksum.
  CompareFolders differs from Linux "md5sum" and "sha1sum" and Windows
  "MD5summer" in that it supports multiple checksums and uses XML format for
  storing checksums in text files.  Other programs for exact file comparisons
  (byte-by-byte) are "comp" on DOS/Windows and "cmp" on Linux; "WinDiff" on
  Windows will compare folders and subfolders.

  Apache License or GNU General Public License
  --------------------------------------------
  CompareFolders3 is free software and has been released under the terms and
  ...
  size avoids thrashing the disk drive.)  CD/DVD drives are faster with smaller
  buffer sizes around 1 KB, and the best size may be larger depending upon the
  speed of your devices.  This is not something that a Java program can
  recognize and compensate for.  If performance is slower than you would
  expect, try changing the "-b" option on the command line, and be very careful
  about judging the results.  Buffer sizes are almost always a power of two:
  1 KB, 2 KB, 4 KB, 8 KB, etc.  By the way, comparing two folders on the same
  CD/DVD disc is not recommended, due to the long latency of "seek" operations
  ...
  5.0 and later packages.  Only simple tags (elements) without attributes are
  recognized.  Everything else is either ignored or will generate an error.
  ...
  confirmed with Java 1.4 through 6 on Windows 2000/XP/Vista/7 and for FAT32
  volumes.  NTFS volumes are not affected.  Both Java and Windows are adjusting
  for DST: Windows uses the current DST offset, and Java tries to be
  ...
  operating systems, including Linux and MacOS.  For maximum compatibility, you
  should avoid all of the following characters: " * / : < > ? \ | and don't
  ...
  (7) Java doesn't report hardware errors, so long as the operating system
      recovers from the error.  You should check event logs after comparing a
      backup folder (on Microsoft Windows or similar), to see if errors were
      detected.  This suggestion is not in the formal documentation, because
      users that know about event logs already do this, and users that don't
      are unlikely to find what they're looking for.  KF, 2014-12-27.
  (8) Whether or not hidden and system files are included in folder listings
      depends upon the whims of the Java run-time environment and the operating
      system.  The global variables <hiddenFlag> and <pathSafeFlag> attempt to
      compensate for this, but are only partially successful and <pathSafeFlag>
      may have the wrong value for some operating systems.  KF, 2015-03-16.
*/
...
public class CompareFolders3
{
  ...
  static final String LIST_COMMA = ", "; // delimiter between items in list
  ...
  /* This program uses CRC32 and MD5 checksums plus one optional member of the
  SHA family.  The default SHA message digest algorithm name is below, and can
  be changed by the -d option on the command line.  For backward compatibility,
  some code here still refers to the SHA family checksum as "SHA1" (without the
  hyphen) in comments, messages, variable names, and parsed data in checksum
  files.  Yes, this is incorrect and may be confusing, but that's what happens
  when you retrofit an old program after a previously "secure" algorithm gets
  broken (SHA1 in 2017).

  Choose <null> for low-security environments (CRC32 and MD5 only, faster),
  "SHA-1" for medium security (normal), and "SHA-256" or "SHA-512" for high
  security (slower).  Relative speeds can be found in comments before our
  calcFileChecksum() method.  Note the exact punctuation.  Everywhere else in
  this program, we casually say "SHA1" (etc), but this must be the official
  form required by the MessageDigest.getInstance() method. */

  static final String SHA_DIGEST_DEFAULT = "SHA-1";
                                  // null "SHA-1" "SHA-256" or "SHA-512"
  ...
  static JCheckBox compContCheckbox; // graphical option for <compContFlag>
  static boolean compContFlag;    // true if we compare file contents, checksums
  static JCheckBox compDateCheckbox; // graphical option for <compDateFlag>
  static boolean compDateFlag;    // true if we compare file dates and times
  static JCheckBox compNameCheckbox; // graphical option for <compNameFlag>
//static boolean compNameFlag;    // true if we compare file names (always)
  static JCheckBox compSizeCheckbox; // graphical option for <compSizeFlag>
  static boolean compSizeFlag;    // true if we compare file sizes (should)
...
//static boolean hiddenFlag;      // true if we process hidden files or folders
  ...
  static String shaDigestName;    // chosen SHA message digest algorithm name
...
*/
  public static void main(String[] args)
  {
    ...
    compContFlag = true;          // by default, compare file contents
    compDateFlag = false;         // by default, don't compare file dates
    compSizeFlag = true;          // by default, compare file sizes
...
//  hiddenFlag = true;            // by default, process hidden files, folders
    ...
    shaDigestName = SHA_DIGEST_DEFAULT; // SHA message digest algorithm name
...
//      bufferReportSize =        // size doesn't change for status updates
      ...
      {
        /* This option is followed by an SHA message digest algorithm name, and
        allows for newer names unknown at the time this program was written. */

        shaDigestName = args[i].substring(2); // may or may not be a valid name
        if (shaDigestName.length() == 0) // special meaning to empty string
        {
          shaDigestName = null;   // disable SHA family checksum
        }
        else try                  // does current run-time recognize this name?
        {
          MessageDigest digest = MessageDigest.getInstance(shaDigestName);
        }
        catch (NoSuchAlgorithmException nsae) // would also catch empty string
        {
          System.err.println("Unknown SHA message digest algorithm name: "
            + args[i]);           // notify user of our arbitrary limits
          showHelp();             // show help summary
          System.exit(EXIT_FAILURE); // exit application after printing help
        }
...
//      hiddenFlag = false;       // ignore hidden files or subfolders
      ...
      else if (word.startsWith("-k") || (mswinFlag && word.startsWith("/k")))
      {
        /* This option is followed by letters to decide which compare options
        are enabled.  Use -k or -kn to choose only file names. */

        compContFlag = compDateFlag = compSizeFlag = false; // disable all
        for (int k = 2; k < word.length(); k ++)
        {
          switch(word.charAt(k))
          {
            case ('c'): compContFlag = true; break; // contents, checksums
            case ('d'): compDateFlag = true; break; // dates, times
            case ('n'): break;    // file names are assumed
            case ('s'): compSizeFlag = true; break; // file sizes
            case ('t'): compDateFlag = true; break; // dates, times
            default:
              System.err.println("Unknown letter \"" + word.charAt(k)
                + "\" for compare options: " + args[i]); // notify user
              showHelp();         // show help summary
              System.exit(EXIT_FAILURE); // exit application
          }
        }
      }
...
//    UIManager.put("Viewport.font", uifont);
    ...
    panel11.add(actionDialog);
    panel11.add(Box.createHorizontalStrut(40));
    panel11.add(Box.createHorizontalGlue()); // some glue inside JComboBox
    ...
    panel11.add(Box.createHorizontalStrut(40));
    panel11.add(Box.createHorizontalGlue());
    ...
    panel21.add(firstFileButton, BorderLayout.WEST);
    ...
    panel21.add(firstFileDialog, BorderLayout.CENTER);
    ...
    panel31.add(secondFileButton, BorderLayout.WEST);
    ...
    panel31.add(secondFileDialog, BorderLayout.CENTER);

    panel01.add(panel31);
    panel01.add(Box.createVerticalStrut(10)); // space between panels

    /* Create a horizontal panel for compare options. */

    JPanel panel41 = new JPanel(new FlowLayout(FlowLayout.CENTER, 0, 0));

    JLabel label42 = new JLabel("Compare:  ");
    if (buttonFont != null) label42.setFont(buttonFont);
    panel41.add(label42);

    compNameCheckbox = new JCheckBox("file names   ", true); // show selected
    if (buttonFont != null) compNameCheckbox.setFont(buttonFont);
    compNameCheckbox.setEnabled(false); // not really an option
    compNameCheckbox.setToolTipText("Select to compare file names.");
//  compNameCheckbox.addActionListener(action); // do last so don't fire early
    panel41.add(compNameCheckbox);

    compSizeCheckbox = new JCheckBox("sizes   ", compSizeFlag);
    if (buttonFont != null) compSizeCheckbox.setFont(buttonFont);
    compSizeCheckbox.setToolTipText("Select to compare file sizes.");
    compSizeCheckbox.addActionListener(action); // do last so don't fire early
    panel41.add(compSizeCheckbox);

    compDateCheckbox = new JCheckBox("dates, times   ", compDateFlag);
    if (buttonFont != null) compDateCheckbox.setFont(buttonFont);
    compDateCheckbox.setToolTipText("Select to compare file dates and times.");
    compDateCheckbox.addActionListener(action); // do last so don't fire early
    panel41.add(compDateCheckbox);

    compContCheckbox = new JCheckBox("contents, checksums", compContFlag);
    if (buttonFont != null) compContCheckbox.setFont(buttonFont);
    compContCheckbox.setToolTipText(
      "Select to compare file contents or checksums.");
    compContCheckbox.addActionListener(action); // do last so don't fire early
    panel41.add(compContCheckbox);
    ...
    JLabel label52 = new JLabel("Options:  ");
    if (buttonFont != null) label52.setFont(buttonFont);
    panel51.add(label52);

    caseCheckbox = new JCheckBox("exact case in file names   ", caseFlag);
    ...
    panel51.add(caseCheckbox);

    scrollCheckbox = new JCheckBox("scroll output   ", scrollFlag);
    ...
    panel51.add(recurseCheckbox);

    panel01.add(panel51);
    panel01.add(Box.createVerticalStrut(10)); // space between panels

    /* Create a horizontal panel for output options. */

    JPanel panel61 = new JPanel();
    panel61.setLayout(new BoxLayout(panel61, BoxLayout.X_AXIS));

//  JLabel label62 = new JLabel("Display: ");
//  if (buttonFont != null) label62.setFont(buttonFont);
//  panel61.add(label62);
//  panel61.add(Box.createHorizontalStrut(5));
    ...
    panel61.add(showFileDialog);
    panel61.add(Box.createHorizontalStrut(40));
    panel61.add(Box.createHorizontalGlue());

//  JLabel label63 = new JLabel("Font: ");
//  if (buttonFont != null) label63.setFont(buttonFont);
//  panel61.add(label63);
//  panel61.add(Box.createHorizontalStrut(5));
    ...
    JPanel panel71 = new JPanel(new FlowLayout(FlowLayout.CENTER, 10, 0));
    panel71.add(panel01);
...
//  outputText.setText("");       // clear output text area
    ...
    JScrollPane panel72 = new JScrollPane(outputText);
    panel72.setBorder(emptyBorder); // no border necessary here
    ...
    JPanel panel81 = new JPanel(new BorderLayout(30, 0));
    ...
    panel81.add(countDialog, BorderLayout.EAST);

    JPanel panel82 = new JPanel(new BorderLayout(0, 0));
    panel82.add(Box.createVerticalStrut(7), BorderLayout.NORTH);
    panel82.add(Box.createHorizontalStrut(10), BorderLayout.WEST);
    panel82.add(panel81, BorderLayout.CENTER);
    panel82.add(Box.createHorizontalStrut(10), BorderLayout.EAST);
//  panel82.add(Box.createVerticalStrut(5), BorderLayout.SOUTH);
    ...
    panel91.add(panel71, BorderLayout.NORTH); // buttons and options
    panel91.add(panel72, BorderLayout.CENTER); // text area
    panel91.add(panel82, BorderLayout.SOUTH); // status message
...
/*
  ...
  Append a second string to the end of a first string, inserting a comma or
  delimiter in between if the first string is not empty.  This is used when
  comparing files to build up lists of features that differ or are the same.
*/
  static String appendComma(String first, String comma, String second)
  {
    if ((first == null) || (first.length() == 0))
      return(second);             // first item does not need delimiter
    else
      return(first + comma + second); // second or following item
  }
...
/*
  ...
  Calculate CRC32, MD5, and one optional SHA family checksum for a given file
  and put those checksums into an object provided by the caller.  We test the
  <cancelFlag> while running, and if that flag is true, we close the file and
  default the checksums to empty strings.

  Speed tests from the FileChecksum Java application are as follows.  If the
  basic file I/O loop takes 13 seconds per gigabyte (GB), then the CRC32
  ...
  seconds.  SHA512 adds 27 seconds.  (These numbers are from Sun Java 6 on
  Microsoft Windows with an Intel Core2 Duo processor at 3.00 GHz.  Your
  results will differ.)
*/
    ...
    CompareFolders3File result)   // and put resulting checksums here
  {
    ...
    MessageDigest sha1digest;     // object for calculating SHA family checksum
    boolean shaFlag;              // true if we do SHA family checksum
    ...
    result.md5 = "";
    result.sha1 = "";
    shaFlag = (shaDigestName != null); // true if we do SHA family checksum
    ...
    try
    {
      ...
      sha1digest = shaFlag ? MessageDigest.getInstance(shaDigestName) : null;
                                  // initialize SHA family message digest
      ...
      while ((i = inStream.read(buffer, 0, bufferChecksumSize)) > 0)
      {
        ...
        if (shaFlag) sha1digest.update(buffer, 0, i); // SHA family checksum
      ...
      if (!cancelFlag)            // don't do more work if cancelled by user
      {
        ...
        method to convert raw bytes to hex, because SHA family checksums need
        the same. */

        result.md5 = formatHexBytes(md5digest.digest());

        /* Convert the SHA family checksum to a hexadecimal string. */

        if (shaFlag) result.sha1 = formatHexBytes(sha1digest.digest());
      }
    }
...
*/
  ...
  static void compareFolderChecksumRecursive(
    ...
    CompareFolders3File secondChecksum) // second calculated checksums
  {
    String compareDiffer;         // which file details are different
    String compareSame;           // which file details are the same
    boolean contentReadFlag;      // true if reading file contents is good
...
//    setStatusMessage("Folder " + pathPrefix + firstFile.getName());
                                  // disabled above: repeats subfolder names
    ...
    {
      /* Compare a first file with the checksum for a second file.  We don't
      compare file names here, as that has already been done before we are
      called.  Calculate the checksum for the first file only if necessary. */

      compareDiffer = "";         // append observed results to these strings
      compareSame = "";
      contentReadFlag = true;     // assume we will read file contents
      firstName = firstFile.getName(); // get name of first file

      if ((compContFlag || compSizeFlag) // contents also depends upon size
        && (secondChecksum.size >= 0))
      {
        /* The checksum has something specified for a file size. */

        if (firstFile.length() == secondChecksum.size)
          compareSame = appendComma(compareSame, LIST_COMMA, "size");
        else
        {
          compareDiffer = appendComma(compareDiffer, LIST_COMMA, "size");
          contentReadFlag = false; // don't waste time reading file contents
        }
      }

      if (compDateFlag && (secondChecksum.date.length() > 0))
      {
        /* The checksum has a non-empty string for the date and time. */

        try                       // date and time could have bad format
        {
          if ((Math.abs(formatDate.parse(secondChecksum.date).getTime() -
            firstFile.lastModified()) < MILLI_FUZZ))
          {
            compareSame = appendComma(compareSame, LIST_COMMA, "date");
          }
          else
          {
            compareDiffer = appendComma(compareDiffer, LIST_COMMA, "date");
//          contentReadFlag = false; // don't waste time reading file contents
          }
        }
        catch (ParseException pe) // checksum has bad date/time format
        {
          compareDiffer = appendComma(compareDiffer, LIST_COMMA,
            "date (error)");
//        contentReadFlag = false; // don't waste time reading file contents
        }
      }

      if (compContFlag && contentReadFlag
        && ((secondChecksum.crc32.length() > 0)
          || (secondChecksum.md5.length() > 0)
          || ((secondChecksum.sha1.length() > 0) && (shaDigestName != null))))
      {
        /* Calculate and compare checksums for the first file against the
        second checksums. */
        ...
        /* We know that most checksums should be present in <firstChecksum>
        because we calculated it.  We don't know this for <secondChecksum>. */

        if ((firstChecksum.crc32.length() > 0)
          && (secondChecksum.crc32.length() > 0))
        {
          /* Both checksums have a non-empty string for the CRC32. */

          if (firstChecksum.crc32.equals(secondChecksum.crc32))
            compareSame = appendComma(compareSame, LIST_COMMA, "CRC32");
          else
            compareDiffer = appendComma(compareDiffer, LIST_COMMA, "CRC32");
        }
          ...
          && (secondChecksum.md5.length() > 0))
        {
          ...
          if (firstChecksum.md5.equals(secondChecksum.md5))
            compareSame = appendComma(compareSame, LIST_COMMA, "MD5");
          else
            compareDiffer = appendComma(compareDiffer, LIST_COMMA, "MD5");
        }

        if ((firstChecksum.sha1.length() > 0)
          && (secondChecksum.sha1.length() > 0))
        {
          /* Both checksums have a non-empty string for the SHA family. */

          if (firstChecksum.sha1.equals(secondChecksum.sha1))
            compareSame = appendComma(compareSame, LIST_COMMA, "SHA");
          else
            compareDiffer = appendComma(compareDiffer, LIST_COMMA, "SHA");
        }
      }
      ...
      {
        putSelectDiff(pathPrefix + firstName + " - different " + compareDiffer
          + ((compareSame.length() > 0) ? (" - same " + compareSame) : ""));
        totalDiffer ++;           // increment number of differences
      }
      else if (compareSame.length() > 0) // was anything the same?
      {
      ...
      {
        putSelectSame(pathPrefix + firstName + " - no data to compare");
        totalSame ++;             // maybe we're just comparing file names?
      }
    }
...
//    setStatusMessage("Folder " + pathPrefix + firstChecksum.name);
        ...
        else                      // both folders still have files/subfolders
        {
              ...
              && firstEntry.name.equalsIgnoreCase(secondEntry.name)))
          {
            ...
            if ((firstEntry.list != null) && (secondEntry.list != null))
            {
              if (recurseFlag)    // does user want us to do subfolders?
              {
                setStatusMessage("Folder " + pathPrefix + firstEntry.name);
    ...
    {
      /* Compare the checksums for two files.  We don't compare file names
      here, as that has already been done before we are called.  Since all
      information is already in the checksums, no attempt is made to avoid
      unnecessary comparisons. */

      compareDiffer = "";         // append observed results to these strings
      compareSame = "";

      if (compSizeFlag && (firstChecksum.size >= 0)
        && (secondChecksum.size >= 0))
      {
        /* Both checksums have something specified for a file size. */
      ...
      if (compDateFlag && (firstChecksum.date.length() > 0)
        && (secondChecksum.date.length() > 0))
      {
        /* Both checksums have a non-empty string for the date and time.  Time
        zones prevent us from comparing these dates and times as strings. */

        try                       // date and time could have bad format
        {
          if (Math.abs(formatDate.parse(firstChecksum.date).getTime()
            - formatDate.parse(secondChecksum.date).getTime()) < MILLI_FUZZ)
          {
            compareSame = appendComma(compareSame, LIST_COMMA, "date");
          }
          else
        ...
        catch (ParseException pe) // one or both have bad date/time format
        {
          compareDiffer = appendComma(compareDiffer, LIST_COMMA,
            "date (error)");
        }
      ...
      if (compContFlag && (firstChecksum.crc32.length() > 0)
        && (secondChecksum.crc32.length() > 0))
      {
        /* Both checksums have a non-empty string for the CRC32. */
      ...
      if (compContFlag && (firstChecksum.md5.length() > 0)
        && (secondChecksum.md5.length() > 0))
      {
        /* Both checksums have a non-empty string for the MD5. */
      ...
      if (compContFlag && (firstChecksum.sha1.length() > 0)
        && (secondChecksum.sha1.length() > 0))
      {
        /* Both checksums have a non-empty string for the SHA family. */
      ...
      {
        putSelectSame(pathPrefix + firstChecksum.name
          + " - no data to compare");
        totalSame ++;             // maybe we're just comparing file names?
      }
    }
...
//    setStatusMessage("Folder " + pathPrefix + firstFile.getName());
    ...
    {
      /* Compare the contents of two files.  We don't compare file names here,
      as that has already been done before we are called. */
      ...
      if (compContFlag || compSizeFlag) // contents also depends upon size
      {
        /* Do these two files have the same size in bytes? */
      ...
      if (compDateFlag)
      {
        /* Do these two files have the same date and time? */

        if (Math.abs(firstFile.lastModified() - secondFile.lastModified())
          < MILLI_FUZZ)
        {
          compareSame = appendComma(compareSame, LIST_COMMA, "date");
        }
        ...
        try
        {
          ...
          while ((cancelFlag == false) && (stopFlag == false))
          {
            ...
            if (firstIndex != secondIndex) // same number of bytes?
            {
              compareDiffer = appendComma(compareDiffer, LIST_COMMA,
                "contents");
              stopFlag = true;    // no, stop comparing
            }
            else if (firstIndex <= 0) // end of file on both files?
            {
              compareSame = appendComma(compareSame, LIST_COMMA, "contents");
    ...
    CompareFolders3File secondChecksum) // second calculated checksums, or null
  {
    ...
    else if (firstFile.isFile())
    {
      ...
      try                         // may cause a parsing exception
      {
        ...
        else if (secondChecksum.crc32.length() == 0) // want CRC32 checksum
        {
          changeFlag = true;      // force new checksum calculation
          totalDiffer ++;         // increment number of differences
          putSelectDiff(pathName + " - missing CRC32 checksum");
        }
        else if (secondChecksum.md5.length() == 0) // want MD5 checksum
        {
          changeFlag = true;      // force new checksum calculation
          totalDiffer ++;         // increment number of differences
          putSelectDiff(pathName + " - missing MD5 checksum");
        }
        else if ((secondChecksum.sha1.length() == 0)
          && (shaDigestName != null)) // optional SHA family checksum
        {
          changeFlag = true;      // force new checksum calculation
          totalDiffer ++;         // increment number of differences
          putSelectDiff(pathName + " - missing SHA family checksum");
        }
        ...
        else if ((secondChecksum.date.length() == 0)
  ...
  static void doActionSelection()
  {
    switch(actionDialog.getSelectedIndex())
    {
      case (ACTION_COMPARE_FOLDERS): // compare two folders
        ...
        compContCheckbox.setEnabled(true);
        compDateCheckbox.setEnabled(true);
        compSizeCheckbox.setEnabled(true);
        ...
        outputText.setText(
      "\nCompare two folders to determine if all files and subfolders are"
      + "\nidentical.  Folders may be on the same computer, on the local"
      + "\nnetwork, or they may be represented by checksum files.  Checksum"
      + "\nfiles are used when the original folders are not available.  A"
      + "\nchecksum file is a plain text file in XML (Extensible Markup"
      + "\nLanguage) format with the name, size, and checksums for each file."
      + "\nChecksums are small hexadecimal \"signatures\" for testing whether"
      + "\nor not files have been copied correctly, such as over a network."
      + "\n\nSelect an action to perform with the top-left button.  Browse for"
      + "\nfiles or folders, or type the full path names.  Click the \"Start\""
      + "\nbutton to begin comparing.  (Click the \"Cancel\" button to stop.)"
      ...
      case (ACTION_CREATE_CHECKSUM): // create checksums for given file/folder
        ...
        compContCheckbox.setEnabled(false);
        compDateCheckbox.setEnabled(false);
        compSizeCheckbox.setEnabled(false);
        ...
        outputText.setText(
      "\nCreate a checksum file for a given folder (or one data file).  Put"
      + "\nthe folder as the first file name.  Output will be written to the"
      + "\nsecond file name as XML text.\n\n");
        break;
      ...
      case (ACTION_FIND_DUPLICATES): // find duplicate files, checksums folders
        ...
        showFileDialog.setEnabled(true);
        outputText.setText(
      "\nTo find duplicate files (equal, identical, same) or unique files"
      + "\n(different) inside a single checksum or folder, put the checksum or"
      + "\nfolder as the first file name.  Don't put a second name.  Select an"
      + "\noption for which files to report."
      + "\n\nTo find duplicate or unique files using two checksums, a checksum"
      + "\nand a folder, or two folders, put the known good or \"trusted\""
      + "\nchecksum or folder as the first file name.  Put the unknown"
      + "\nchecksum or folder as the second file name.  (Duplicate files"
      + "\ninside the \"trusted\" folder are ignored.)\n\n");
        break;
      ...
      case (ACTION_UPDATE_CHECKSUM): // update checksum with new folder data
        ...
        showFileDialog.setEnabled(true);
        outputText.setText(
      "\nUpdate a checksum file by scanning a given folder (or one data file)"
      + "\nand assuming that files with the same name, size, and date have not"
      + "\nchanged.\n\n");
        break;
...
/*
  ...
  we look for any identical file in the first list, that is not the same File
  object, of course.  Then if the file from the second list is not already in
  the first list, we add it for later comparisons.
*/
    ...
    Vector orderList)             // sorted list of files in linear order
  {
    ...
    for (orderIndex = 0; orderIndex < orderLength; orderIndex ++)
    {
      ...
      {
        /* Find first if any match for <orderEntry> in <sizeList>. */

        orderFound = false;       // assume <orderEntry> not in <sizeList>
        sameFound = false;        // assume there are no duplicate files
        sameText = null;          // no message yet about duplicate match
        ...
        for (sizeIndex = 0; sizeIndex < sizeLength; sizeIndex ++)
        {
          ...
          else
          {
            ...
            if ((cancelFlag == false) && (orderEntry.object != null)
              && ((orderEntry.md5.length() == 0) // required MD5 checksum
              || ((orderEntry.sha1.length() == 0) && (shaDigestName != null))))
                                  // optional SHA family checksum
            {
            ...
            if ((cancelFlag == false) && (sizeEntry.object != null)
              && ((sizeEntry.md5.length() == 0) // required MD5 checksum
              || ((sizeEntry.sha1.length() == 0) && (shaDigestName != null))))
                                  // optional SHA family checksum
            {
              calcFileChecksum(sizeEntry.object, sizeEntry);
            }

            /* Compare checksums.  Look only for positive results here. */

            if (cancelFlag)
              { /* do nothing */ }
            else
            {
              /* For both the MD5 and the SHA family, there are three possible
              cases: missing (can't compare), different, or equal.  Combined,
              this gives nine cases, only five of which have positive matches.
              The "MD5 (not SHA)" case can be easily generated with checksum
              files created using two different SHA digest names.  The "SHA
              (not MD5)" case would take a cosmic coincidence (or cheating). */

              int index = 0;      // add 0,1,2 for MD5 and 0,3,6 for SHA family

              if ((orderEntry.md5.length() == 0) // MD5 checksum
                || (sizeEntry.md5.length() == 0))
              {
                /* Do nothing if one or both checksums are missing. */
              }
              else if (orderEntry.md5.equals(sizeEntry.md5))
                index = 2;        // same MD5 checksum
              else index = 1;     // different MD5 checksum

              if ((orderEntry.sha1.length() == 0) // SHA family checksum
                || (sizeEntry.sha1.length() == 0))
              {
                /* Do nothing if one or both checksums are missing. */
              }
              else if (orderEntry.sha1.equals(sizeEntry.sha1))
                index += 6;       // same SHA family checksum
              else index += 3;    // different SHA family checksum

              switch (index)      // five cases have positive matches
              {
                case (2): sameText = "MD5"; break; // match MD5, missing SHA
                case (5): sameText = "MD5 (not SHA)"; break; // only match MD5
                case (6): sameText = "SHA"; break; // match SHA, missing MD5
                case (7): sameText = "SHA (not MD5)"; break; // only match SHA
                case (8): sameText = "MD5, SHA"; break; // match both MD5, SHA
              }

              if (sameText != null) // were any matches found?
              {
                sameFound = true; // two distinct files have same checksum
                sameText = "same " + sameText + " as " + sizeEntry.name;
              }
            }
          }
...
/*
  makeCommentText() method

  This method converts a string parameter to text acceptable in XML comments,
  which do not allow two consecutive hyphens.  Some other characters are also
  escaped on general principle.
*/
  static String makeCommentText(String input)
  {
    StringBuffer buffer;          // faster than String for multiple appends
    char ch;                      // one character from input string
    int count;                    // number of consecutive hyphens
    final String HYPHEN = "&#45;"; // replacement for unprintable hyphens
    int i;                        // index variable
    int length;                   // size of input string in characters

    buffer = new StringBuffer();  // allocate empty string buffer for result
    count = 0;                    // no consecutive hyphens found yet
    length = input.length();      // get size of input string in characters
    for (i = 0; i < length; i ++) // do all characters in caller's string
    {
      ch = input.charAt(i);       // get one character from input string
      if (ch == '-')              // is this a possibly dangerous hyphen?
        count ++;                 // yes, count number of consecutive hyphens
      else                        // no, character is not a hyphen
      {
        if (count == 1) buffer.append('-'); // if exactly one pending hyphen
        else while (count -- > 0) buffer.append(HYPHEN); // multiple hyphens
        count = 0;                // all pending hyphens are now done

        if ((ch < 0x20)           // control codes are unprintable
          || (ch >= 0x7F)         // anything past plain text is unprintable
          || (ch == '&')          // ampersand is character escape delimiter
          || (ch == '<')          // less and greater than are XML delimiters
          || (ch == '>'))
    ...
    /* All trailing hyphens must be escaped, because even a single hyphen could
    get appended by the official "-->" ending for an XML comment, resulting in
    an early double hyphen. */

    while (count -- > 0) buffer.append(HYPHEN); // unprintable trailing hyphen

    return(buffer.toString());    // give caller our converted string

  } // end of makeCommentText() method
    ...
    throws IOException            // comes from calling readChecksumToken()
  {
    ...
    if (first.equals("<file>"))
    {
      ...
      while ((cancelFlag == false) && (second.equals("</file>") == false))
      {
        ...
        else if (second.equals("<sha>")) // SHA family checksum, may be null
        {
          /* Newer checksum files use <sha>...</sha> tags for all SHA family
          checksums. */
          ...
          {
            cancelFlag = true;
            putError("Missing checksum after file <sha> at line "
              + formatComma.format(readFileLine) + ": " + third);
          }
          else                    // we have input that should be a checksum
          {
            ...
            {
              cancelFlag = true;
              putError("Invalid hex checksum after file <sha> at line "
                + formatComma.format(readFileLine) + ": " + third);
            }
            ...
            {
              fourth = readChecksumToken(input); // get next input token
              if (fourth.equals("</sha>") == false)
              {
                cancelFlag = true;
                putError("Missing </sha> after SHA family checksum at line "
                  + formatComma.format(readFileLine) + ": " + fourth);
              }
            }
          }
          if (shaDigestName == null) result.sha1 = ""; // don't want SHA family
        }
        else if (second.equals("<sha1>")) // SHA1 checksum, may be null
        {
          /* Older checksum files used <sha1>...</sha1> tags for SHA1 specific
          checksums.  We read these as SHA family checksums. */
          ...
          if (shaDigestName == null) result.sha1 = ""; // don't want this SHA1
        }
        else
        {
          cancelFlag = true;
...
*/
  static void setStatusMessage(String text)
  {
    ...
    String gap = "   ";           // spaces or text between counter items
    ...
    {
      if (!empty) buffer.append(gap); // space between items
...
*/
  static void showHelp()
  {
    ...
    System.err.println();
    System.err.println("Create CRC32, MD5, SHA checksums on standard output.  Syntax is:");
    ...
    System.err.println("  -d# = SHA message digest algorithm name: SHA-1 SHA-256 or SHA-512");
    System.err.println("      default is: -d" + SHA_DIGEST_DEFAULT);
    System.err.println("  -k# = compare file contents (c), dates (d), sizes (s); default is -kcs");
...
*/
  static void userButton(ActionEvent event)
  {
    ...
    else if (source == compContCheckbox) // compare file contents, checksums
    {
      compContFlag = compContCheckbox.isSelected();
    }
    else if (source == compDateCheckbox) // compare file dates and times
    {
      compDateFlag = compDateCheckbox.isSelected();
    }
    else if (source == compSizeCheckbox) // compare file sizes
    {
      compSizeFlag = compSizeCheckbox.isSelected();
    }
...
//    writeCheckSumLine(output,
//      "<!-- http://kwfenske.github.io/ -->");
      if (source.length() > 0)    // was there an original source given?
        writeCheckSumLine(output, "<!-- original source: "
          + makeCommentText(source) + " -->");
...
*/
    ...
    throws IOException            // comes from calling writeCheckSumLine()
  {
    ...
    {
      /* Newer checksum files use <sha>...</sha> tags for all SHA family
      checksums.  Older checksum files used <sha1>...</sha1> tags specific to
      SHA1 checksums.  Hence, older checksum files can't be compared as text
      with newer checksum files, even if they both have SHA1 checksums. */
      ...
      if (fileinfo.date.length() > 0) // file date and time (if any)
        writeCheckSumLine(output, (nextindent + "<date>" + fileinfo.date
          + "</date>"));
      if (fileinfo.size >= 0)     // file size in bytes (if any)
        writeCheckSumLine(output, (nextindent + "<size>" + fileinfo.size
          + "</size>"));
      if (fileinfo.crc32.length() > 0) // CRC32 checksum (if any)
        writeCheckSumLine(output, (nextindent + "<crc32>" + fileinfo.crc32
          + "</crc32>"));
      if (fileinfo.md5.length() > 0) // MD5 checksum (if any)
        writeCheckSumLine(output, (nextindent + "<md5>" + fileinfo.md5
          + "</md5>"));
      if (fileinfo.sha1.length() > 0) // SHA family checksum (if any)
      {
        writeCheckSumLine(output, (nextindent + "<sha>" + fileinfo.sha1
          + "</sha>"));
//      writeCheckSumLine(output, (nextindent + "<sha1>" + fileinfo.sha1
//        + "</sha1>"));          // old tags for SHA1 specific checksums
      }
    ...
    else                          // must be a folder
    {
      ...
      if (fileinfo.name.length() > 0) // folder name (if any)
      {
        writeCheckSumLine(output, (nextindent + "<name>"
          + makeCharReference(fileinfo.name) + "</name>"));
      }
...
class CompareFolders3File implements Comparable
{
  ...
  String sha1;                    // SHA family checksum
  ...
  public CompareFolders3File()
  {
    ...
    this.sha1 = "";               // default to empty SHA family checksum
  }
